...
-(void)doMainThreadStuff{
    ...
    if (!jsonArray) {
        ...
        for(NSDictionary *item in self->jsonArray) {
            ...
            //Now needs to convert from GMT to the phone's time:
            NSDateFormatter *df = [[NSDateFormatter alloc] init];
            [df setDateFormat:@"yyyy-MM-dd*HH:mm:ss"];
            NSDate *utcDate = [df dateFromString: messageTime];
            ...
            NSDate* ts_utc_string = [df_utc dateFromString:messageTime];
            NSString* ts_local_string = [df_local stringFromDate:ts_utc_string];
            NSDate *myDate = [df dateFromString:ts_local_string];
            NSLog(@"TS LOCAL STRING: %@",ts_local_string);
...
-(void)addChatToDB:(NSString *)message{
    ...
    dispatch_async(fetchQ, ^{
        ...
        //Old message for phone generated times:
        ...
        NSString *urlText = [NSString stringWithFormat:@"http://hungrylikethewolves.com/serverlets/addchatmessagejson.php?session=%@&message=%@",token,strippedMessage];
